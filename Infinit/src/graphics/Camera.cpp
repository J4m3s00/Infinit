...
namespace Infinit {
	...
	void Camera::Update()
	{
		if (Input::IsKeyPressed(IN_KEY_LEFT_ALT))
		{
			const glm::vec2& mouse{ Input::GetMouseX(), Input::GetMouseY() };
			glm::vec2 delta = mouse - m_InitialMousePosition;
			m_InitialMousePosition = mouse;

			if (Input::IsMousePressed(IN_MOUSE_BUTTON_MIDDLE))
				MousePan(delta);
			else if (Input::IsMousePressed(IN_MOUSE_BUTTON_LEFT))
				MouseRotate(delta);
			else if (Input::IsMousePressed(IN_MOUSE_BUTTON_RIGHT))
				MouseZoom(delta.y);
		}
